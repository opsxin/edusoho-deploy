...
ensure_user_exist() {
    egrep "^${running_user}" /etc/group >& /dev/null
    if [ $? -ne 0 ]; then
        groupadd ${running_user}
    fi

    egrep "^${running_user}" /etc/passwd >& /dev/null
    if [ $? -ne 0 ]; then
        useradd -g ${running_user} ${running_user}
    fi
}
...
EOF
    ...
    sed -i -e '$iclient_max_body_size 1024M;' \
        -e "s/user  nginx;/user ${running_user} ${running_user};/" /etc/nginx/nginx.conf
...
install_mysql() {
    if [ "${os}" == "centos" -a "${os_version}" -eq 7 ]; then
    ...
    elif [[ "${os}" == "ubuntu" && "${os_version}" -ge 2004 ]]; then
...
install_edusoho() {
    ...
    chown -R ${running_user}:${running_user} "/var/www/edusoho"
}
